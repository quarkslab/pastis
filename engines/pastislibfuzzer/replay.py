...
class Replay(object):
    ...
    def run(binary_path: str, args: List[str] = [], stdin_file=None, timeout=None, cwd=None):
        ...
        replay._process = subprocess.Popen([binary_path]+args, stdin=stdin, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=cwd)
        try:
    ...
    def is_asan_without_crash(self) -> bool:
        ...
        return bool(self._asan_bugtype) and not self.has_crashed()

    @property
    def returncode(self) -> int:
        assert self._process is not None, "Replay process not started"
