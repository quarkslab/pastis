...
class BrokingMode(Enum):
    ...
    FUZZER_ONLY = 4       # New seeds are shared between fuzzers, but not wih the DSE engines
    DSE_RANDOM_FILTER = 5 # Only a random subset of new seeds are sent to the DSE engines

DSE_SHARING_PROB = 0.1
...
class PastisBroker(BrokerAgent):
    ...
    def seed_received(self, cli_id: bytes, typ: SeedType, seed: bytes):
        ...
        # Avoid flooding DSE engines with new seeds found by the fuzzers
        elif self.broker_mode == BrokingMode.FUZZER_ONLY:
            for c in self.iter_other_clients(cli):
                if c.engine != FuzzingEngine.TRITON\
                and c.is_new_seed(seed):
                    self.send_seed(c.netid, typ, seed)  # send the seed to the client
                    c.add_seed(seed)

        # Only a random subset of new seeds are sent to the DSE
        elif self.broker_mode == BrokingMode.DSE_RANDOM_FILTER:
            for c in self.iter_other_clients(cli):
                if c.engine == FuzzingEngine.TRITON:
                    p = random.random()
                    if p > DSE_SHARING_PROB:
                        continue
                if c.is_new_seed(seed):
                    self.send_seed(c.netid, typ, seed)  # send the seed to the client
                    c.add_seed(seed)
    ...
    def hello_received(self, cli_id: bytes, engines: List[Tuple[FuzzingEngine, str]], arch: Arch, cpus: int, memory: int, hostname: str):
        ...
        if self.running: # A client is coming in the middle of a session
            ...
            if self.broker_mode in [BrokingMode.FULL, BrokingMode.FUZZER_ONLY]:
            ...
            # Randomly filter out some seeds to avoid flooding the DSE
            if self.broker_mode == BrokingMode.DSE_RANDOM_FILTER:
                for seed, typ in self._seed_pool.items():
                    if client.engine == FuzzingEngine.TRITON:
                        p = random.random()
                        if p > DSE_SHARING_PROB:
                            continue
                    self.send_seed(client.netid, typ, seed)  # necessarily a new seed
    ...
    def start_client(self, client: PastisClient):
        ...
        for eng, _ in engines.most_common()[::-1]:
            ...
            if eng == FuzzingEngine.HONGGFUZZ:
                # Try finding a suitable binary for the current engine and the client arch
                program = self.programs.get((client.arch, eng, ExecMode.HF_PERSISTENT))
                if program:
                    exmode = ExecMode.HF_PERSISTENT  # a program was found in persistent mode
            ...
            # Add static input corpus directory and complog if possible
            if engine == FuzzingEngine.AFL:
                if engine_args == "":
                    engine_args += f"-p fast -d -t1000 -m none -L30 " #some dummy config
                # engine_args += "-s 12345678 " # Fix seed for test runs
                engine_args += f"-i {self.workspace / self.INPUT_DIR} "
                cmplog = self.programs.get((client.arch, eng, ExecMode.AFL_CMPLOG))
                if cmplog is not None:
                    engine_args += f"-c {cmplog} "
            elif engine == FuzzingEngine.AFL_UNICORN:
                if engine_args == "":
                    engine_args += f"-U -p fast -d -t1000 -m none -L30 " #some dummy config
                # engine_args += "-s 12345678 " # Fix seed for test runs
                engine_args += f"-i {self.workspace / self.INPUT_DIR} "
                cmplog = self.programs.get((client.arch, eng, exmode))
                if cmplog is not None:
                    engine_args += f"-c {cmplog} "
            elif engine == FuzzingEngine.AFL_QEMU:
                if engine_args == "":
                    engine_args += f"-Q -p fast -d -t1000 -m none -L30 " #some dummy config
                engine_args += f"-i {self.workspace / self.INPUT_DIR} "
                cmplog = self.programs.get((client.arch, eng, exmode))
                if cmplog is not None:
                    engine_args += f"-c {cmplog} "
    ...
    def _find_binary_workspace(self, binaries_dir) -> None:
        ...
        for file in d.iterdir():
            if file.is_file():
                ...
                funcs = [f.name for f in p.symbols]
                if '__klocwork_alert_placeholder' in funcs:
                    klw = True
                if {'__afl_maybe_log', '__afl_trace'} & set(funcs):
                    engine = FuzzingEngine.AFL
                elif 'uc_afl_emu_start' in funcs:
                    engine = FuzzingEngine.AFL_UNICORN
                elif 'hfuzzInstrumentInit' in funcs:
                    engine = FuzzingEngine.HONGGFUZZ
                else:
                    engine = FuzzingEngine.TRITON

                if not klw:
                    logging.warning(f"binary {file} does not contain klocwork intrinsics")
                ...
                if engine == FuzzingEngine.HONGGFUZZ:
                    ...
                    if '.rodata' in sections:
                        ...
                        if self.HF_PERSISTENT_SIG in rodata_content:
                            exmode = ExecMode.HF_PERSISTENT
                    else:
                        if 'HF_ITER' in (x.name for x in p.imported_functions):  # More dummy method
                            exmode = ExecMode.HF_PERSISTENT
                elif engine == FuzzingEngine.AFL:
                    #if set(['__cmplog_ins_hook%d'%i for i in [1,8,16,32,64]]) & set(funcs):
                    #TODO For now there is no clean way to discriminate a CMPLOG binary without disassembling it
                    if  file.name[-4:] == ".cmp": # UGLY UGLY UGLY UGLY UGLY
                        exmode = ExecMode.AFL_CMPLOG
                ...
                if arch:
                    ...
                    if tup in self.programs:
                        ...
                        #duplicate for QEMU mode
                        if engine == FuzzingEngine.TRITON:
                            tup = (arch, FuzzingEngine.AFL_QEMU, exmode)
                            self.programs[tup] = dst_file
